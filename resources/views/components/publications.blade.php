@if (Lang::has('publications') && !empty(Lang::get('publications')))
    <section class="section publications">
        @include('snippets.background')
        <div class="container section__container">
            <div class="section__left">
                <div class="section__content">
                    @if (Lang::has('publications.title') && !empty(Lang::get('publications.title')))
                        <h3 class="section__title">{{ __('publications.title') }}</h3>
                    @endif
                </div>
            </div>
                                    ...
                                    @if (!empty($item['title']))
                                        ...
                                        >
                                            @if(!empty($item['language']['code']) && app()->getLocale() != $item['language']['code'])
                                                ...
                                                <br>
                                                <span class="publications__title-original">({{ __('publications.list.' . $key . '.title', [], $item['language']['code']) }})</span>
                                            @else
                                                {{ $item['title'] }}
                                            @endif
                                        </a>
                                    ...
                                    @if (!empty($item['heading']))
                                        <p class="publications__heading">
                                            @if(!empty($item['heading']) && app()->getLocale() != $item['language']['code'])
                                                ...
                                                <br>
                                                <span>({{ __('publications.list.' . $key . '.heading', [], $item['language']['code']) }})</span>
                                            @else
                                                {{ $item['heading'] }}
                                            @endif
                                        </p>
                                    @endif
                                    @if (!empty($item['info']) || !empty($item['language']))
                                        <p class="publications__info">
                                            @if (!empty($item['info'])){{ $item['info'] }}@endif
                                            @if (!empty($item['language']['text']))({{ $item['language']['text'] }})@endif
                                        </p>
                                    @endif
                                </li>
                            @endforeach
